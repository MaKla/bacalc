			...
			if(i == 0){
				...
				if(checkValidChar(step) == NUMBER){
					...
					if(i != validationForm.length()-1){
						...
						// and now previous is a faculty or NAN
						else if(checkValidChar(validationForm.at(i-1)) == FACULTY || checkValidChar(validationForm.at(i-1)) == NAN){
							...
							while (j!=0) {
								...
								else if(checkValidChar(validationForm.at(j)) == OPER){
									// if previous is a non valid sign next loop
									if(checkValidChar(validationForm.at(j-1)) == NAN || checkValidChar(validationForm.at(j-1)) == FACULTY){
										j--;
									} 
